...
import type { MRT_Row, MRT_RowData, MRT_SortingFn, MRT_TableInstance } from "material-react-table";
...
export const onExport = (table: MRT_TableInstance<GreenITIndicateur>) => {
    ...
    const csvData: string[] = filteredRows.map(row =>
        [
            ...
            `"${row.original.lettreGreen}"`,
            `"${row.original._conso}"`,
            `"${row.original._cpu}"`,
            `"${row.original._ram}"`,
            `"${row.original._disk}"`,
            `"${row.original.gaspillage}"`,
            `"${row.original.consoNormalized}"`,
            `"${row.original.impactNormalized}"`,
            `"${row.original._nbVm}"`
        ].join(",")
    );

    handleExportCsv("green-it", HEADERS_GREEN_IT, csvData);
};
